...
open class DynamicDelegate: NSObject, Cancellable {
...
extension NSObject {
	fileprivate func setBinderStorage(_ newValue: BinderStorage?) {
...
public func embedStorageIfInUse<Storage: BinderStorage>(_ instance: NSObject, _ storage: Storage, _ cancellables: [Cancellable]) {
	...
	if storage.inUse {
		assert(objc_getAssociatedObject(instance, &associatedStorageKey) == nil, "Bindings should not be applied twice to the same instance.")
